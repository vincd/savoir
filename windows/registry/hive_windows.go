...
func (key WindowsKey) EnumKey() ([]HiveKey, error) {
	...
	for i := uint32(0); i < keyInfo.SubKeyCount; i++ {
		...
		subKey, err := registry.OpenKey(key.key, name, registry.QUERY_VALUE|registry.ENUMERATE_SUB_KEYS)
		if err != nil {
			return nil, err
		}
...
func (h WindowsHive) OpenKey(path string) (HiveKey, error) {
	...
	k, err := registry.OpenKey(h.Key, name, registry.QUERY_VALUE|registry.ENUMERATE_SUB_KEYS)
	if err != nil {
