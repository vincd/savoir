...
func (p Page) search(handle windows.Handle, pattern []byte) (binary.Pointer, error) {
...
func NewProcessReaderWithHandle(name string, pId uint32, handle windows.Handle) (*ProcessReader, error) {
	...
	modules, err := psapi.EnumAllProcessModules(p.handle)
	if err != nil {
		return nil, err
	}
...
func NewProcessReader(processName string) (*ProcessReader, error) {
	...
	handle, err := windows.OpenProcess(windows.PROCESS_VM_READ|windows.PROCESS_QUERY_LIMITED_INFORMATION, false, entry.ProcessID)
	if err != nil {
		return nil, err
	}
