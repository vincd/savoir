...
func GuessNextTag(bytes []byte) (int, error) {
	t, _, err := parseTagAndLength(bytes, 0)
	if err != nil {
		return 0, err
	}
...
func parseTagAndLength(bytes []byte, initOffset int) (ret tagAndLength, offset int, err error) {
	...
	b := bytes[offset]
	// fmt.Printf("parseTagAndLength: TAG/CLASS bytes[%d]=%x\n", offset, b)
	offset++
	ret.class = int(b >> 6)
	...
	b = bytes[offset]
	// fmt.Printf("parseTagAndLength: LENGTH bytes[%d]=%x\n", offset, b)
	offset++
	if b&0x80 == 0 {
		...
		for i := 0; i < numBytes; i++ {
			...
			ret.length <<= 8
			ret.length |= int(b)
			// SAVOIR: Remove minimal length check
			/*if ret.length == 0 {
	...
	// fmt.Printf("parseTagAndLength: len: %d (%x)\n", ret.length, ret.length)
...
func parseSequenceOf(bytes []byte, sliceType reflect.Type, elemType reflect.Type) (ret reflect.Value, err error) {
	// fmt.Printf("parseSequenceOf: %s / %s\n", sliceType, elemType)
	...
	for offset := 0; offset < len(bytes); {
		...
		// fmt.Printf("TEST 3: %b / %d %d / %d %d\n", matchAny, t.class, ClassUniversal, t.tag, expectedTag)
...
func parseField(v reflect.Value, bytes []byte, initOffset int, params fieldParameters) (offset int, err error) {
	...
	if ifaceType := fieldType; ifaceType.Kind() == reflect.Interface && ifaceType.NumMethod() == 0 {
		...
		if !t.isCompound && t.class == ClassUniversal {
			...
			// Savoir: https://github.com/jcmturner/gofork/blob/dc7c13fece037a4a36e2b3c69db4991498d30692/encoding/asn1/asn1.go#L639
	...
	if params.explicit {
		...
		// fmt.Printf("TEST: %s %s / %s %s\n", t.class, expectedClass, t.tag, *params.tag)
	...
	if !params.explicit && params.tag != nil {
		// fmt.Printf("!params.explicit && params.tag != nil\n")
		expectedClass = ClassContextSpecific
	...
	if !params.explicit && params.application && params.tag != nil {
		// fmt.Printf("!params.explicit && params.application && params.tag != nil\n")
		expectedClass = ClassApplication
		expectedTag = *params.tag
		matchAnyClassAndTag = false
	}
	...
	if !params.explicit && params.private && params.tag != nil {
		// fmt.Printf("!params.explicit && params.private && params.tag != nil\n")
	...
	// fmt.Printf("TEST2: %b / %d %d / %d %d / %s\n", matchAnyClassAndTag, t.class, expectedClass, t.tag, expectedTag, v.Addr().Interface())
	if !matchAnyClassAndTag && (t.class != expectedClass || t.tag != expectedTag) || (!matchAny && t.isCompound != compoundType) {
		// Tags don't match. Again, it could be an optional element.
		ok := setDefaultValue(v, params)
		// fmt.Printf("setDefaultValue returns: %b\n", ok)
		if ok {
			offset = initOffset
		} else {
	...
	// fmt.Printf("SWITCH ON : %+v\n", v.Addr().Interface())
	switch v := v.Addr().Interface().(type) {
	case *RawValue:
		// fmt.Printf("THIS IS RAW_VALUE\n")
	...
	case reflect.Struct:
		// fmt.Printf("THIS IS A STRUCTURE: %+v\n", v)
		...
		for i := 0; i < structType.NumField(); i++ {
			...
			// fmt.Printf("Parse field: %d (%s)/%d (%+v)\n", i, field.Name, structType.NumField(), parseFieldParameters(field.Tag.Get("asn1")))
